...
dHdx = @(x,p,al,et,mu,nu) ...
      [B*(1-al)*((1-al)*x(2)+x(5))*(p(2)-p(1))+(MU-mu)*(x(4)+x(5)+x(6))*(p(4)-p(1))-d*p(1);
       B*((1-al)^2*x(1)*(p(2)-p(1))+(1-al)*x(4)*(p(5)-p(4)))+(MU-mu)*(x(4)+x(5)+x(6))*(p(5)-p(2))+(g+et)*(p(3)-p(2))-d*p(2)+c1;
       -d*p(3)+(MU-mu)*(x(4)+x(5)+x(6))*(p(6)-p(3));
       B*((1-al)*x(2)+x(5))*(p(5)-p(4))-d*p(4)+(MU-mu)*(x(1)*(p(4)-p(1))+x(2)*(p(5)-p(2))+x(3)*(p(6)-p(3)))+nu*(p(1)-p(4))+c2;
